 ...
 * @author AliRS
 */
public class Test_Climbing3 extends SimpleApplication {
    ...
    public static void main(String[] args) {
        ...
        Test_Climbing3 app = new Test_Climbing3();
...
//        setupSky();
    ...
    private void setupPlayer() {
        ...
        // setup model
        player = (Node) assetManager.loadModel(CHARACTER_MODEL);
        player.setName("Player");

        Node animRoot = (Node) AnimUtils.getAnimRoot(player);
        animRoot.attachChild(debugShapes.drawAxis());

        // vertical
        Node ledgeRayV = new Node("LedgeRayV");
        ledgeRayV.attachChild(debugShapes.drawWireCube(0.1f, ColorRGBA.Red));
        animRoot.attachChild(ledgeRayV);
        ledgeRayV.setLocalTranslation(FVector.forward(animRoot).multLocal(0.5f).addLocal(0, 3, 0));

        // horizontal
        Node ledgeRayH = new Node("LedgeRayH");
        ledgeRayH.attachChild(debugShapes.drawWireCube(0.1f, ColorRGBA.Blue));
        animRoot.attachChild(ledgeRayH);
        ...
        playerControl.ledgeRayH = ledgeRayH;
        playerControl.ledgeRayV = ledgeRayV;
        playerControl.bcc = bcc;
        animRoot.addControl(playerControl);

        animRoot.addControl(bcc);
        physics.getPhysicsSpace().add(bcc);

        // setup third person camera
        setupChaseCamera(animRoot);

        rootNode.attachChild(player);

        configureRootMotion(animRoot);

        // debug update order
        for (int i = 0; i < animRoot.getNumControls(); i++) {
            System.out.println((i + 1) + " " + animRoot.getControl(i).getClass().getSimpleName());
        }
    }
    
    private void configureRootMotion(Spatial sp) {
        ...
        Joint hips = skinningControl.getArmature().getJoint("mixamorig:" + MixamoBodyBones.Hips);
        ...
        TransformTrack hipsTrack = MyAnimation.findJointTrack(climbing, hips.getId());
        ...
        // Note, applying it to "animRoot" node will cause a strange side effect. Probably because BCC is also controlling it!
        TransformTrack climbingRootMotionTrack = new TransformTrack(sp, hipsTrack.getTimes(), translations, null, null);

        animComposer.addAction(AnimDefs.Climbing, new BaseAction(
                sequence(
                        // Activate root motion physics
                        //callMethod(bcc, "setEnabled", false),
                        callMethod(bcc.getRigidBody(), "setKinematic", true),
                        //callMethod(bcc, "setApplyPhysicsLocal", true),
                        parallel(
                        ...
                        callMethod(bcc.getRigidBody(), "setKinematic", false),
                        //callMethod(bcc, "setApplyPhysicsLocal", false),
                        // Make sure to remove our self now
                        callMethod(animComposer, "removeCurrentAction"),
                        callMethod(pc, "onClimbingDone")
                )));
    }
    ...
    private class PlayerControl extends AdapterControl implements ActionListener {
        ...
        public void setSpatial(Spatial sp) {
            super.setSpatial(sp);
            if (spatial != null) {
//                this.bcc = getComponent(MyBetterCharacterControl.class);
                this.animComposer = getComponent(AnimComposer.class);
            }
        }
        ...
        void onClimbingStart() {
            isClimbingMode = true;
            startClimb = true;
            isClimbingAnimDone = false;
            System.out.println("onClimbingStart");
        }

        void onClimbingDone() {
            isClimbingAnimDone = true;
            bcc.warp(goalPosition);
            System.out.println("onClimbingDone");
        }
